...
| PLP (end-node) | `5030:728` | — | Captured 2026-03-30. File key: `LXJLDI1fRXble63hVJcg7A`. NO CollectionHero. Breadcrumbs → sidebar+grid layout. FilterSidebar as card (`bg-white border rounded-[12px] w-[240px]`): Category links w/ count badges, Price Range inputs+Apply, teal checkboxes. Results header: count+subtitle LEFT, Filters+Sort btns RIGHT. ActiveFilterChips (teal pills). 4-col `'end-node'` ProductCard grid (250px image). Plan: `plans/PLP_END_NODE_PAGE_PLAN.md`. |
| PLP (category/non-end-node) | `5024:284` | — | Captured 2026-03-30. File key: `LXJLDI1fRXble63hVJcg7A`. Hero 280×200px image+desc, SubcategoryScrollSection (horizontal scroll, 120×120px squares), 6-col `'category'` ProductCard grid, sort-only header (no filter sidebar). Conditional on `subcollections.length > 0`. Plan: `plans/PLP_CATEGORY_PAGE_PLAN.md`. |
...
| Return Process — Step 3 (Shipping) | `1:656` | — | Captured 2026-03-28. File key: `NEk59QDkRMC3a6LD0WZVQJ`. 3 shipping options (UPS Drop-off, Pickup, Instant Return), return address card, packing instructions, summary box, tracking notice. 5 new components: ShippingOptionCard, ReturnAddressCard, PackingInstructions, ReturnShippingSummary, ReturnTrackingNotice. Plan: `plans/RETURN_SHIPPING_PLAN.md`. |
| Return Process — Step 4 (Make It Right) | `1:1030` | — | Captured 2026-03-28. File key: `NEk59QDkRMC3a6LD0WZVQJ`. 2×2 resolution grid (Exchange, Replace, Store Credit, Refund), selected items preview thumbnails, resolution summary (inside card), "Our Promise" notice. 4 new components: ResolutionOptionCard, SelectedItemsPreview, ResolutionSummary, OurPromiseNotice. Plan: `plans/RETURN_RESOLVE_PLAN.md`. |
